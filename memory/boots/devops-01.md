...
## Звітність
Після виконання:
1. Дописати у `/memory/log.md` запис `## <дата> — DevOps — devops-01`: що зроблено, що змінилось (URL репо, URL staging, провайдер), блокери.
2. Оновити у `/memory/state.md` ТІЛЬКИ рядок DevOps: статус `boot виконано`, "Останній завершений" = devops-01. І перенести відомі тепер факти (URL репо, staging, Supabase регіон) з розділу "Що НЕ визначено" у "Що є в репо зараз".
3. Усі коміти з префіксом `[devops-01]`.

## Наступний крок
Якщо виявиш потребу в новому boot (наприклад, "devops-02: prod-середовище" або "backend-01 потребує X від інфри") — напиши пропозицію у `/memory/boots/proposed/<назва>.md` за тим самим шаблоном. Не виконуй її сам. Після звіту — зупинись і чекай наступного boot від Orchestrator.
